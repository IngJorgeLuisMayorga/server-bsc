  ...
  <tr class="email--top pink" style="
    ...
    <td colspan="3">
            ...
            @if ($status === 's1_ordered')
                ...
                <h2 style="text-align: center">{{ $order->user_name }}!</h2>
                </div>
            ...
            @elseif ($status === 's3_delivered')
                ...
                padding: 8px 0px;
                width: fit-content;
                text-align: center;">{{ $order->user_name }}!</h2>
                </div>
            @else
...
">
    <td colspan="3" style="width: 100%;padding: 20px 60px;">
        ...
        @if ($status === 's1_ordered')
                <h4 class="label--date"><strong>Pedido recibido</strong>  {{ date('d F , Y', strtotime($order-> updated_at)) }} </h4>
            @elseif ($status === 's2_shipped')
                <h4 class="label--date"><strong>Pedido enviado</strong>  {{ date('d F , Y', strtotime($order-> updated_at)) }}</h4>
            @elseif ($status === 's3_delivered')
                <h4 class="label--date"><strong>Pedido entregado</strong>  {{ date('d F , Y', strtotime($order-> updated_at)) }} </h4>
            @else
                    ...
                    <td colspan="2" style="vertical-align: top; padding-top: 0px;">
                        ...
                        @if ($item->discount > 0)
                            <h2 style="font-size: 30px; text-align: left;">
                                @if (TRUE )
                                 <span style="font-size: 20px;"> NAN x </span>
                                @endif
                ...
                <td colspan="1">
                    <div style="position: relative; left: 55px">
                        ...
                        <div class="box box-pink" style="width: 100%; max-width:100%; padding-right: 90px">
                            <h1 style="font-weight: 300; font-size:30px">Total<strong> @money($order->order_total) </strong> </h1>
                        </div>
